...
public interface VideoDetailRepository extends JpaRepository<VideoDetail, Long> {
    ...
    @Query("select v from VideoDetail v where v.fileName in ?1 and v.category = ?2")
    List<VideoDetail> findByFileNameIsInAndCategoryEquals(Collection<String> fileNames, String category);
}
